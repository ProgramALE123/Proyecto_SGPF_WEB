...
export const login = async ({ usuario, clave }) => {
  ...
  const token = jwt.sign({ sub: account.id, username: account.nombre_usuario, role: account.rol }, secret(), { expiresIn: process.env.JWT_EXPIRES_IN || '8h', issuer: 'sgpf-backend', audience: 'sgpf-frontend' });
